...
public function viewCatProducts($id) {
      ...
      $products = DB::select("SELECT u.*, p.* FROM products AS u INNER JOIN pro_images AS p ON p.pro_id = ( SELECT pro_id FROM pro_images AS p2 WHERE p2.prod_img_prod_id = u.prod_id AND p2.prod_default_status = '1' ORDER BY pro_id ASC LIMIT 1 ) where u.prod_cate_id = '".$id."' AND u.prod_live_status = '1' AND u.prod_quantity > '0' AND u.prod_status = 'Active'");
...
public function viewRelatedProducts($id) {
   ...
   $result = DB::table('products')->select('prod_cate_id')->where('prod_id', $id)->first();
      ...
      $relPro =   DB::select("SELECT u.*, p.* FROM products AS u INNER JOIN pro_images AS p ON p.pro_id = ( SELECT pro_id FROM pro_images AS p2 WHERE p2.prod_img_prod_id = u.prod_id AND p2.prod_default_status = '1' ORDER BY pro_id ASC LIMIT 1 ) where u.prod_cate_id = $catId AND u.prod_id NOT IN ($id) LIMIT 4");
...
public function viewProductImages($id) {
    ...
    $prodImg = DB::table('pro_images')
                ...
                ->orderBy('prod_default_status', 'DESC')
                ->get();
...
public function stripePostMobPayment(Request $request) {
      ...
      $var = json_decode($request->cartItems);
        ...
        Stripe\Stripe::setApiKey('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');
      ...
      $arr=[];
       ...
       foreach($var as $cartinsrt){
        ...
        if(count($sellerComission) == '0'){
          $comission= ($cartinsrt->prod_price * 5) / 100;
          if($comission >= 25){
            $comission = 25;
          } else {
            $comission = $comission;
...
public function editPassword(Request $request){
  ...
  public function viewHome() {
      ...
      $newArrivals = DB::select("SELECT u.*, p.* FROM products AS u INNER JOIN pro_images AS p ON p.pro_id = ( SELECT pro_id FROM pro_images AS p2 WHERE p2.prod_img_prod_id = u.prod_id AND u.prod_live_status = '1' AND u.prod_quantity > '0' AND u.prod_status = 'Active' AND p2.prod_default_status = '1' ORDER BY pro_id ASC LIMIT 1 )ORDER BY u.prod_date_time DESC LIMIT 40"); 
...
foreach($orders as $new){
  ...
  $newArrivals = DB::select("SELECT u.*, p.* FROM products AS u INNER JOIN pro_images AS p ON p.pro_id = ( SELECT pro_id FROM pro_images AS p2 WHERE p2.prod_img_prod_id = u.prod_id AND u.prod_id = '".$new->prod_id."' AND p2.prod_default_status = '1' ORDER BY pro_id ASC LIMIT 1 )ORDER BY u.prod_date_time DESC LIMIT 40"); 
