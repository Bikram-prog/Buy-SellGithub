   ...
   public function addMoreProductForm($id){
    ...
    $produniqid = hash('sha256', $mailunq);
      ...
      $product_info = new Product(); 
        ...
        $product_info->prod_delivery_days = request('dlvry_days');
        ...
        $product_info->prod_mmbr_sctn = request('mmbr_cndtn');
        $product_info->prod_mmbr_price = request('mmbr_price');
        $product_info->prod_mmbr_quantity = request('mmbr_minm_quantity');
        ...
        if(request('sbcatgry_id') == '10'){
          return redirect('/selleraddmeasurement/'.$uniq_id)->with(['prodsucmsg' => 'Your Product Added Sucessfully.']);
        }

        

        if(request('cate_id') == '2'){
          return redirect('prodattributes/'.$uniq_id.'/'.request('cate_id'))->with(['prodsucmsg' => 'Your Product Added Sucessfully.']);
          // return redirect('variation/'.$uniq_id.'/'.request('cate_id'))->with(['prodsucmsg' => 'Your Product Added Sucessfully.']);
        }else{
          return redirect('prodattributes/'.$uniq_id.'/'.request('cate_id'))->with(['prodsucmsg' => 'Your Product Added Sucessfully.']);
    ...
    public function addVariationPost(Request $request){

      

      $implode= implode(',', $request->input('meta_val'));

      $var_all= DB::table('variations')
                ->where('variations.var_prod_id','=', $request->input('prod_id'))
                ->where('variations.var_value','=', $implode)
                ->get();

                

      if(count($var_all) == 0){
        DB::insert('insert into variations (var_prod_id, var_value, var_quantity, var_price) values (?, ?, ?, ?)', [$request->input('prod_id'), $implode, $request->input('quantity'), $request->input('price')]);
      }

      
      
      return redirect('variation/'.request('prod_id').'/'.request('cate_id'))->with(['prodsucmsg' => 'Your Product Added Sucessfully.']);

    }

    public function sellerVariation($id,$catid){

      $variaton= DB::table('variation_meta')
                 ->where('variation_meta.var_meta_cat_id','=', $catid)
                 ->get();

      $var_all= DB::table('variations')
                ->where('variations.var_prod_id','=', $id)
                ->get();
      
      return view('productvariation')->with(['variaton'=> $variaton, 'id'=> $id, 'catid'=> $catid, 'var_all'=> $var_all]);

    }

    public function delProdVariation($id){
    ...
    public function imgUpload($id){
      ...
      $imgCount= count($ProImage);
      $uploadCount= 20-$imgCount;
      ...
      $image = count($ProImage);
        ...
        return view('img1')->with(['lstprodid' => $id, 'category'=>$category, 'ProImage'=>$ProImage, 'image'=>$image, 'finish'=>$finish,'uploadCount' =>$uploadCount]);
    }

    public function ProdImage($id){
      ...
      return response()->json(['ProImage'=>$ProImage]);
    }
    ...
    public function liveProduct($id){
     ...
     public function imgUpload1(){
      ...
      public function fileStore(Request $request){
        ...
        $ProImage = ProImage::where('prod_img_prod_id', '=', [$request->input('prod_id')])->get();
...
if($imgCount < 20){
 ...
 return response()->json(['success'=>'Your Photo Uploaded Successfully']);
  ...
  return response()->json(['error'=>'Maximum 20 images are allowed.']);
}
    ...
    public function defaultImage(Request $request, $id, $prod_id){
    
      
      DB::table('pro_images')->where('pro_id', $id )->update(['prod_default_status' => '1']);
    ...
    public function addMeasurementForm($id){
      ...
      $category = DB::table('category')->get();
       ...
       $totalearning =  DB::select("SELECT SUM(ord_paid_amount) AS TotalIEarning FROM orders WHERE ord_seller_id = '".$id."' AND ord_status ='Delivered'");
        ...
        return view('addmeasurement',['uniqid'=>$id, 'totalearning'=>$totalearning, 'category'=>$category]);
    }

    public function addMeasurement(Request $request){

      DB::table('products')->where('prod_id', $request->input('proid') )->update(['prod_wmn_bust' => $request->input('bust'),'prod_wmn_waist' => $request->input('waist'),'prod_wmn_bottom' => $request->input('bottom'),'prod_wmn_armpit' => $request->input('armpit'),'prod_wmn_shoulder' => $request->input('shoulder')]);


      $uniqid = $request->input('proid');
      
      return redirect('/sellerimgupld/'.$uniqid);

  }

    //// Seller Edit Product Start ////

    public function editMeasurementForm($id){
      ...
      $category = DB::table('category')->get();
       ...
       $totalearning =  DB::select("SELECT SUM(ord_paid_amount) AS TotalIEarning FROM orders WHERE ord_seller_id = '".$id."' AND ord_status ='Delivered'");
        ...
        return view('editmeasurement',['Editpro'=>$Editpro, 'totalearning'=>$totalearning, 'category'=>$category]);
    }
    ...
    $this->validate($request,[
      'cate_id'=>'required',
        ...
        'prod_cndtn'=>'required',
        'quantity'=>'required|numeric:value',
        'dlvry_days'=>'required|numeric:value',
        'lg_desc'=>'required|max:5000',
        // 'tags'=>'required',
        //'price'=>'required|numeric:value',
        'offrprice'=>'required|numeric:value'
        ...
        DB::table('products')->where('prod_id', $request->input('proid') )->update(['prod_country' => $request->input('contry_id'),'prod_cndtn' => $request->input('prod_cndtn'),'prod_cate_id' => $request->input('cate_id'),'prod_sub_cate_id' => $request->input('sbcatgry_id') , 'prod_title' => $request->input('title'),'prod_long_desc' => $request->input('lg_desc'),'prod_quantity' => $request->input('quantity'),'prod_delivery_days' => $request->input('dlvry_days'),'prod_price' => $request->input('offrprice'),'prod_tags' => $request->input('tags'),'prod_seller_id' => $request->input('seller_id'),'prod_mmbr_sctn' => $request->input('mmbr_cndtn'),'prod_mmbr_price' => $request->input('mmbr_price'),'prod_mmbr_quantity' => $request->input('mmbr_minm_quantity')]);


        $Editpro = $request->input('proid');

        if(request('sbcatgry_id') == '10'){
          return redirect('/sellereditmeasurement/'.$Editpro)->with(['prodsucmsg' => 'Your Product Added Sucessfully.']);
        }

        if(request('cate_id') == '2'){
          return redirect('/editprodattributes/'.$Editpro)->with(['prodsucmsg' => 'Your Product Added Sucessfully.']);
          // return redirect('editvariation/'.$Editpro.'/'.request('cate_id'))->with(['prodsucmsg' => 'Your Product Added Sucessfully.']);
        }else{
          return redirect('/editprodattributes/'.$Editpro)->with(['prodsucmsg' => 'Your Product Added Sucessfully.']);
          // return redirect('/editimage/'.$Editpro)->with(['prodsucmsg' => 'Your Product Added Sucessfully.']);
        }
    ...
    public function editSellerVariation($id,$catid){
      ...
      return view('edit_productvariation')->with(['variaton'=> $variaton, 'id'=> $id, 'catid'=> $catid, 'var_all'=> $var_all]);

    }

    public function addEditVariationPost(Request $request){
      ...
      return redirect('editvariation/'.request('prod_id').'/'.request('cate_id'))->with(['prodsucmsg' => 'Your Product Added Sucessfully.']);
    ...
    public function delAttributes($id,$prodid){
               ...
               DB::table('variations')->where('var_prod_id', $prodid)->delete();
                
              //  return redirect('/editprodattributes/'.$prodid)->with(['delprod'=>'Product Inactivated successfully']);
              return Redirect::back()->with(['delprod'=>'Product Inactivated successfully']);
    ...
    public function editImageForm($id){
      ...
      $image = count($ProImage);
        return view('editimage')->with(['lstprodid' => $id, 'ProImage'=>$ProImage, 'category'=>$category, 'payment'=>$payment, 'image'=>$image, 'uploadCount'=>$uploadCount]);
    }

    public function editProdImage($id){
...
public function stripePendingPayment(Request $request){
  ...
  $category = DB::table('category')->take(10)->get();
    ...
    try{
      ...
      }elseif(!isset($_COOKIE['UserCompName'])){
        return redirect('/buyerlogin');
      }else{
        $category = DB::table('category')->take(10)->get();
      ...
      return $th->getMessage();
            ...
            public function trackingNumberPost(Request $request){
              ...
              DB::table('orders')->where('ord_id', $request->input('order_id') )->update(['ord_tracking_no' => $request->input('track_no'), 'ord_carrier' => $request->input('carrier')]);

              $order= DB::table('orders')
                      ->leftJoin('products', 'products.prod_id', '=', 'orders.order_prod_id')
                      ->where('orders.ord_id', '=', $request->input('order_id'))
                      ->first();

                      

              $orduser= DB::table('seller_users')->where('s_id', $order->ord_buyer_id)->first();
              
              $orduseradd= DB::table('buyer_addresses')->where('buyer_del_add_buy_id', $order->ord_buyer_id)->where('make_primary', '1')->first();
              ...
              $mail2= $orduseradd->buyer_del_email;
              

              Mail::send(['html'=>'email_ship_order'], ['order'=>$order, 'prodImg'=>$prodImg, 'orduser'=>$orduser], function($message) use ($mail,$mail2)  {
                
                $message->to($mail,$mail2)->subject
                    ('Your Order has now been Shipped - Buy & Sell');
...
public function productRelist($id){
    ...
    $product_info = new Product(); 
    $product_info->prod_country = $products[0]->prod_country;
    $product_info->prod_cate_id = $products[0]->prod_cate_id;
    $product_info->prod_cndtn = $products[0]->prod_cndtn;
    $product_info->prod_sub_cate_id = $products[0]->prod_sub_cate_id;
    $product_info->prod_title = $products[0]->prod_title;
    $product_info->prod_long_desc = $products[0]->prod_long_desc;
    //$product_info->prod_tags = request('tags');
    $product_info->prod_quantity = $products[0]->prod_quantity;
    $product_info->prod_delivery_days = $products[0]->prod_delivery_days;
    //$product_info->prod_reg_price = request('price');
    $product_info->prod_price = $products[0]->prod_price;
    $product_info->prod_seller_id = Crypt::decryptString($_COOKIE['UserIds']);
    $product_info->prod_id = $produniqid;
    
    $product_info->save();

    $products_img = DB::table('pro_images')
              ->where('prod_img_prod_id', '=', $id)
              ->get();

    foreach($products_img as $pro_img){
    
      $imageUpload = new ProImage();
      $imageUpload->prod_img_prod_id = $produniqid;
      $imageUpload->pro_img_path = $pro_img->pro_img_path;
      $imageUpload->prod_default_status = $pro_img->prod_default_status;
      $imageUpload->save();

    }

    $products_var = DB::table('variations')
              ->where('var_prod_id', '=', $id)
              ->get();

    foreach($products_var as $pro_var){

      DB::insert('insert into variations (var_prod_id, var_value, var_quantity, var_price) values (?, ?, ?, ?)', [$produniqid, $pro_var->var_value, $pro_var->var_quantity, $pro_var->var_price]);

    }

return redirect('/sellerdashboard')->with('succmsg','Product Relisted Successfully.');
 }
