...
<div class="container-xl mt-4 px-4 ">
        <div class="row gx-5">
                    ...
                    <div class="col-md-5" style="background-color: #fff; padding: 20px; border-radius: 10px; border: 1px solid #eee; ">
                    @if (isset($_COOKIE['UserIds']) && Crypt::decryptString($_COOKIE['UserCompName']) != '' && $user->prod_seller_id != Crypt::decryptString($_COOKIE['UserIds']) && $prodQuantity > 0 && $prodStatus != "Inactive")
                ...
                //$offerpercnt = 100-(($user->prod_price*100)/$user->prod_reg_price); 
                        ...
                        <form action="{{ route('cart') }}" method="GET">
                            ...
                            <input type="hidden" value="{{ $user->prod_price }}" id="price" name="price">
...
@if (isset($_COOKIE['UserIds']) && $user->prod_seller_id != Crypt::decryptString($_COOKIE['UserIds']) &&$prodQuantity > 0 && $prodStatus != "Inactive" && count($watch) > 0 )
                        <div class="row mt-4">
                            <div class="col-md-6">
                             ...
                             <div class="col-md-6">
                                <div class="d-grid gap-2">
                                    <?php if(isset($_COOKIE['UserEmail'])) { ?>
                                        <button onclick="loadChat('0', 'Customer support', 'customer@support.globallove', 'offline')" id="idd_0" type="button" class="btn btn-success btn-md d-none d-sm-block"><i class="fas fa-phone-alt"></i> Have a question about this item?</button>
                                        <?php } else { ?>
                                        <button onclick="alert('You must sign in or signup as a Buy & Sell member before you can use this chat facility');" type="button" class="btn btn-success btn-md d-none d-sm-block"><i class="fas fa-phone-alt"></i> Have a question about this item?</button>
                                        <?php } ?>
                                </div>
                            </div> 
                        </div>
                        @elseif (isset($_COOKIE['UserIds']) && $user->prod_seller_id != Crypt::decryptString($_COOKIE['UserIds']) &&$prodQuantity > 0 && $prodStatus != "Inactive" && count($watch) == 0)
                        ...
                        <div class="row mt-4">
                            <div class="col-md-6">
                             ...
                             <div class="col-md-6">
                                <div class="d-grid gap-2">
                                    <?php if(isset($_COOKIE['UserEmail'])) { ?>
                                        <button onclick="loadChat('0', 'Customer support', 'customer@support.globallove', 'offline')" id="idd_0" type="button" class="btn btn-danger btn-md "><i class="fas fa-phone-alt"></i> Have a question about this item?</button>
                                        <?php } else { ?>
                                        <button onclick="alert('You must sign in or signup as a Buy & Sell member before you can use this chat facility');" type="button" class="btn btn-danger btn-md "><i class="fas fa-phone-alt"></i> Have a question about this item?</button>
                                        <?php } ?>
                                </div>
                            </div> 
                        </div>

                        @elseif(!isset($_COOKIE['UserIds']) && count($watch) == 0)
    ...
    <div class="container-xl mt-4">
        <div class="row">
            <div class="col-12">
                <h2 style="font-weight: bold; font-size: 22px; margin-bottom: 20px;">You may also like</h2>
              <div class="row">
                <?php $j = 0; ?>
                @foreach ($relPro as $rel)
                <div class="col-md-3">
                  <div class="card box">
                    <a href="/p/{{ $rel->prod_id }}"><img style="object-fit: cover; height: 300px;" src="{{asset('images')}}/{{ $rel->pro_img_path }}" class="card-img-top" alt="name"></a>
                    <div class="card-body">
                      <!-- <h5 class="card-subtitle mb-2">Electronics</h5> -->
                      <a href="/p/{{ $rel->prod_id }}" style="text-decoration: none;"><h5 class="card-title mb-3">{{ $rel->prod_title }}</h5></a>
                      <p class="card-text price mb-2">${{ $rel->prod_price }}</p>
                      <p style="font-weight: 500;"><span class="badge bg-warning"><?php echo number_format($ratings_rel[$j][0]->AverageRatings, 1); ?> <i class="fas fa-star"></i></span> <small></small></p>
